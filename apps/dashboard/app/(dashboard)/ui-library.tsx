...
const swatches = [
  ["Primary", colors.primary], ["Accent", colors.accent], ["Canvas", colors.background], ["Surface", colors.surface], ["Text", colors.text], ["Muted", colors.textMuted], ["Success", colors.success], ["Warning", colors.warning], ["Danger", colors.danger], ["Info", colors.info],
] as const;
...
export default function UiLibraryPage() {
  ...
  return <PageScaffold eyebrow="Design system" title="UI Library" description="A living reference for Ody’s tokens, reusable components, and product states.">
    ...
    <LibrarySection number="02" title="Typography" description="A compact scale designed for dense operational information."><Card><TypeRow label="Display / 36" size={fontSizes.display} weight={fontWeights.extraBold}>Restaurant overview</TypeRow><Divider /><TypeRow label="Heading 1 / 28" size={fontSizes.heading1} weight={fontWeights.extraBold}>Good afternoon, Dina</TypeRow><Divider /><TypeRow label="Heading 2 / 22" size={fontSizes.heading2} weight={fontWeights.bold}>Recent orders</TypeRow><Divider /><TypeRow label="Body / 14" size={fontSizes.body} weight={fontWeights.regular}>Keep your team moving with clear, current information.</TypeRow><Divider /><TypeRow label="Caption / 12" size={fontSizes.caption} weight={fontWeights.semibold}>UPDATED 2 MINUTES AGO</TypeRow></Card></LibrarySection>
...
function LibrarySection({ number, title, description, children }: React.PropsWithChildren<{ number: string; title: string; description: string }>) {
  return <View style={styles.section}><View style={styles.sectionHeading}><Text style={styles.number}>{number}</Text><View><Text style={styles.title}>{title}</Text><Text style={styles.description}>{description}</Text></View></View>{children}</View>;
}

function TypeRow({ label, size, weight, children }: React.PropsWithChildren<{ label: string; size: number; weight: typeof fontWeights[keyof typeof fontWeights] }>) {
  return <View style={styles.typeRow}><Text style={styles.typeLabel}>{label}</Text><Text style={{ flex: 1, color: colors.text, fontSize: size, lineHeight: Math.round(size * 1.25), fontWeight: weight }}>{children}</Text></View>;
}

const styles = StyleSheet.create({
  section: { marginBottom: spacing[12] }, sectionHeading: { flexDirection: "row", alignItems: "flex-start", gap: spacing[3], marginBottom: spacing[5] }, number: { color: colors.primary, fontSize: fontSizes.micro, fontWeight: fontWeights.extraBold, marginTop: 5 }, title: { color: colors.text, fontSize: fontSizes.heading2, fontWeight: fontWeights.extraBold }, description: { color: colors.textMuted, fontSize: fontSizes.caption, marginTop: spacing[1] }, swatches: { flexDirection: "row", flexWrap: "wrap", gap: spacing[3] }, swatch: { width: 116, padding: spacing[2], borderWidth: 1, borderColor: colors.border, borderRadius: radii.medium, backgroundColor: colors.surface }, color: { height: 70, borderRadius: radii.small, borderWidth: 1, borderColor: "rgba(0,0,0,.06)" }, swatchName: { color: colors.text, fontSize: fontSizes.caption, fontWeight: fontWeights.bold, marginTop: spacing[2] }, code: { color: colors.textSubtle, fontSize: fontSizes.micro, fontFamily: "monospace", marginTop: 2 }, typeRow: { flexDirection: "row", alignItems: "baseline", gap: spacing[6] }, typeLabel: { width: 110, color: colors.textSubtle, fontSize: fontSizes.micro, fontFamily: "monospace" }, twoColumns: { flexDirection: "row", flexWrap: "wrap", alignItems: "flex-start", gap: spacing[4] }, flexCard: { flex: 1, minWidth: 310 }, spacingList: { gap: spacing[3] }, spacingRow: { flexDirection: "row", alignItems: "center", gap: spacing[3] }, tokenLabel: { width: 70, color: colors.textMuted, fontSize: fontSizes.caption }, spacingBar: { height: 12, borderRadius: radii.small, backgroundColor: colors.primary }, surfaceDemo: { marginBottom: spacing[4] }, body: { color: colors.textMuted, fontSize: fontSizes.body }, darkSurface: { padding: spacing[5], borderRadius: radii.large, backgroundColor: colors.surfaceStrong }, darkTitle: { color: colors.textInverse, fontSize: fontSizes.heading3, fontWeight: fontWeights.extraBold, marginTop: spacing[4] }, darkBody: { color: palette.stone400, fontSize: fontSizes.caption, marginTop: spacing[1] }, row: { flexDirection: "row", alignItems: "center", flexWrap: "wrap", gap: spacing[2] }, form: { gap: spacing[4] }, stateGrid: { flexDirection: "row", flexWrap: "wrap", gap: spacing[4] }, stateCard: { flex: 1, minWidth: 240 }, dialogActions: { flexDirection: "row", justifyContent: "flex-end", gap: spacing[2] },
});
